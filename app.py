...
st.markdown("""
    ...
    div.stButton > button { background-color: #2d6a4f !important; color: white !important; border-radius: 10px; font-weight: bold; height: 3em; }
    [data-testid="stSidebar"] { background-color: #1b4332; }
    [data-testid="stSidebar"] * { color: white !important; }
    .content-card { background-color: rgba(255, 255, 255, 0.98); padding: 25px; border-radius: 15px; color: #1b4332; }
    </style>
...
if menu == "🏠 Início":
    ...
    if "auth" not in st.session_state:
        ...
        if not dados.empty:
            f = st.selectbox("Filtrar:", ["Todos"] + especies_lista)
            ...
            for i, r in df_f.iterrows():
                with st.expander(str(r['Nome'])):
                    ...
                    # Linha do botão simplificada para evitar erro de parênteses
                    link_final = "https://wa.me/55" + str(r['Contato']).replace(".0","").strip()
                    st.link_button("💬 WhatsApp", link_final)
        st.markdown("</div>", unsafe_allow_html=True)